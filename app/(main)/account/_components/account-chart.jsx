...
const AccountChart = ({ transactions = [] }) => {  // ✅ Fix: Default to empty array if undefined

    const [dataRange, setDataRange] = useState("1M");

    const filteredData = useMemo(() => {
        if (!transactions || transactions.length === 0) return [];  // ✅ Fix: Early return if no data
                    ...
                    <Select defaultValue={dataRange} onValueChange={setDataRange}>
                        ...
                        <SelectContent>
                            {Object.entries(DATE_RANGES).map(([key, { label }]) => (
                                <SelectItem key={key} value={key}>  {/* ✅ Fix: Use `key` instead of `label` */}
                                    {label}
                                </SelectItem>
                            ))}
                        </SelectContent>
