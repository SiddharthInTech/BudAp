    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        if (
          ...
          console.log(
            `🟡 Budget Alert Triggered for user: ${budget.user.email}`
          );
          ...
          console.log("✅ lastAlertSent updated for", budget.user.email);
        }
      });
    }
  }
...
export const triggerRecurringTransactions = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    if (recurringTransactions.length > 0) {
      ...
      //3.Send evencts to be processed
...
export const generateMonthlyReports = inngest.createFunction(
  {
    id: "generate-mothly-reports",
    name: "Generate Monthly Reports",
  },
  { cron: "0 0 1 * *" },
  async ({ step }) => {
    const users = await step.run("getch-users", async () => {
...
async function generateFinancialInsights(stats, month) {
  ...
  const prompt = `
    ...
    - Expense Categories: ${Object.entries(categories)
